...
public class ReviewPaging {
	...
	public ReviewPaging(int totalCount, int curPage) {
	...
	public ReviewPaging(int totalCount, int curPage, int listCount, int pageCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	private void makePaging() {
		...
		if( listCount == 0 )	setListCount(5);	//화면에 보여질 게시글 수 10개로 기본 설정
