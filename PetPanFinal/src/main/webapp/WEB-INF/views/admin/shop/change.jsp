...
<div id = "container">
	<div id = "center-div">
		...
		<form action = "<%=request.getContextPath() %>/admin/shop/change" method = "post" id = "content-form" enctype="multipart/form-data">
			...
			<input type = "text" name = "name" id = "title" placeholder = " 제목을 입력하세요" required = "required" value="${shop.name }"></input>
			<div align="left" style= "float:left;">
			<label> 수량   
			<input type = "number" name = "remain" id = "numbox" required = "required" value="${shop.remain }">
			</label>
			</div>
			<div align="right">
			<label> 가격   
			<input type = "number" name = "price" id = "numbox" required = "required" value="${shop.price}">
			</label>
			</div>
			<br>
			<textarea id = "content" name = "shopcontent" required = "required">${shop.shopcontent }</textarea>
			...
			<c:forEach var="shopFile" items="${shopFile}" varStatus="length">
				<input type = "hidden" value="${shopFile.fileno }" name="delete">
			<tr>
				<td><input type = "hidden" value="${length.count -1 }" name="save">${shopFile.originname }</td>
